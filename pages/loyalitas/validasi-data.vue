...
type IPendaftar = {
  ...
  pembekalan_kewirausahaan: string;
  sertifikat: string;
  uang_penghargaan: string;
  keterangan: string;
};
...
const pendaftar = reactive<IPendaftar[]>([
  {
    ...
    pembekalan_kewirausahaan: "Sudah",
    sertifikat: "Sudah Dapat",
    uang_penghargaan: "Sudah",
    keterangan: "Ybs. Sudah Mendapatkan Sertifikat",
  },
  {
    ...
    cltn_tmt: "10 Mei 2023",
    tgl_pengangkatan: "10 Mei 2023",
    pembekalan_kewirausahaan: "Belum",
    sertifikat: "Belum Dapat",
    uang_penghargaan: "Belum",
    keterangan: "Tidak Diikutsertakan Pembekalan Kewirausahaan DIKARENAKAN YBS. MERUPAKAN PENGAWAS SMA/SMK",
  },
  {
...
const selectedComponentData = computed(() => {
  if (selectedComponent.value === 'usulan') {
    return {
      title: 'Usulan Validasi Data',
      description: 'Data usulan validasi data aparatur sipil negara Pemerintah Provinsi Jawa barat',
    };
  } else if (selectedComponent.value === 'verifikasi') {
    return {
      title: 'Verifikasi Data',
      description: 'Data verifikasi validasi data aparatur sipil negara Pemerintah Provinsi Jawa barat',
    };
  } else if (selectedComponent.value === 'riwayat') {
    return {
      title: 'Riwayat Validasi Data',
      description: 'Data riwayat validasi data aparatur sipil negara Pemerintah Provinsi Jawa barat',
    };
  }
});
...
<template>
  <section>
    ...
    <div class="mt-5">
      <nav class="flex flex-col sm:flex-row">
        ...
        <button :class="[
          ...
          {
            'text-green-600 border-b-2 border-green-600': selectedComponent === 'verifikasi',
          },
        ]" @click="showComponent('verifikasi'); resetNilai(); updateBreadcrumb('Verifikasi Data')">
          Verifikasi Data
        </button>
        <button :class="[
          'text-gray-600 py-4 px-6 block hover:text-green-600 focus:outline-none',
        ...
        ]" @click="showComponent('riwayat'); resetNilai(); updateBreadcrumb('Riwayat')">
          Riwayat
        </button>
      </nav>
    </div>
    ...
    <div v-if="selectedComponent === 'verifikasi'">
      <div class="mt-5 flex flex-col gap-4">
        <div class="flex items-center">
          <label class="flex items-center">
            <input type="checkbox" class="form-checkbox h-5 w-5 text-green-500 ml-2" v-model="selectAllChecked" @change="handleSelectAll" />
        ...
        <table class="w-full text-sm text-left text-gray-800 border rounded-lg border-separate border-spacing-0">
          <thead class="text-sm text-white font-bold font-roboto capitalize bg-green">
            <tr class="[&>*]:border-b">
              ...
              <th scope="col" class="px-2 py-2.5">Nama</th>
              <th scope="col" class="px-2 py-2.5">NIP</th>
              <th scope="col" class="px-2 py-2.5">Pembekalan Kewirausahaan</th>
              <th scope="col" class="px-2 py-2.5">Sertifikat</th>
              <th scope="col" class="px-2 py-2.5">Uang Penghargaan Loyalitas</th>
              <th scope="col" class="px-2 py-2.5">Keterangan/Catatan</th>
              <th scope="col" class="px-2 py-2.5">Perangkat Daerah</th>
              <th scope="col" class="px-2 py-2.5">Jabatan</th>
              <th scope="col" class="px-2 py-2.5">Tgl Pengangkatan</th>
              <th scope="col" class="px-2 py-2.5">Usia</th>
          ...
          <tbody class="[&>*:nth-child(even)]:bg-gray-50">
            <tr v-for="(item, i) in pendaftar" :key="i" class="[&:not(:last-child)>*]:border-b [&:last-child>*:first-child]:rounded-bl-lg [&:last-child>*:last-child]:rounded-br-lg">
              ...
              <td class="px-2 py-2.5">{{ item.nip }}</td>
              <td class="px-2 py-2.5">
                <FormVueSelect v-model="item.pembekalan_kewirausahaan" />
              </td>
              <td class="px-2 py-2.5">
                <FormVueSelect v-model="item.sertifikat" />
              </td>
              <td class="px-2 py-2.5">
                <FormVueSelect v-model="item.uang_penghargaan"/>
              </td>
              <FormTextArea :rows="2" required />
              <td class="px-2 py-2.5">{{ item.perangkat_daerah }}</td>
              <td class="px-2 py-2.5">{{ item.jabatan }}</td>
              <td class="px-2 py-2.5">{{ item.tgl_pengangkatan }}</td>
              <td class="px-2 py-2.5">{{ item.usia }}</td>
        ...
        <table class="w-full text-sm text-left text-gray-800 border rounded-lg border-separate border-spacing-0">
          ...
          <tbody class="[&>*:nth-child(even)]:bg-gray-50">
            <tr v-for="(item, i) in pendaftar" :key="i" class="[&:not(:last-child)>*]:border-b [&:last-child>*:first-child]:rounded-bl-lg [&:last-child>*:last-child]:rounded-br-lg">
              ...
              <td class="px-2 py-2.5">{{ item.nama }}</td>
              <td class="px-2 py-2.5">{{ item.nip }}</td>
              <td class="px-2 py-2.5">{{ item.pembekalan_kewirausahaan }}</td>
              <td class="px-2 py-2.5">{{ item.sertifikat }}</td>
              <td class="px-2 py-2.5">{{ item.uang_penghargaan }}</td>
              <td class="px-2 py-2.5">{{ item.keterangan }}</td>
              <td class="px-2 py-2.5">{{ item.perangkat_daerah }}</td>
              <td class="px-2 py-2.5">{{ item.jabatan }}</td>
              <td class="px-2 py-2.5">{{ item.tgl_pengangkatan }}</td>
              <td class="px-2 py-2.5">{{ item.usia }}</td>
