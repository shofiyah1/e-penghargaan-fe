...
const loyalitas = reactive<loyalitas[]>([
  {
    nama: 'File Merge Berkas Persyaratan',
  },
...
const isUbahBerkasMode = ref(false);
...
<template>
  <ModalBaseDetail :is-open="show" :title="title" @close="closeModal">
    ...
    <div v-if="selectedComponent === 'datadiri'">
      <tr>
        <td class="px-2 py-2.5 rounded-lg h-5 flex items-center justify-center bg-green-100 text-green-600 border border-green-150 mt-5" style="display: inline-flex; margin-right: 10px;">
          Sudah Melengkapi Berkas
        </td>
        <td class="px-2 py-2.5 rounded-lg h-5 flex items-center justify-center bg-blue-100 text-blue-600 border border-blue-150" style="display: inline-flex;margin-right: 10px;">
          Sudah Diperiksa
        </td>
        <td class="px-2 py-2.5 rounded-lg h-5 flex items-center justify-center bg-yellow-100 text-yellow-600 border border-yellow-150" style="display: inline-flex;">
          Perbaikan Berkas
        </td>
      </tr>
      <AlertDanger class="mt-5">
      ...
      <div class="flex gap-4">
        <div class="w-1/2">
          <div class="mb-4">
            <label for="tmtpangkat" class="block mb-2">TMT Jabatan</label>
            <FormTextAreaDisabled v-model="data.tmt_jabatan" :rows="1" name="tmtpangkat" id="tmtpangkat" readonly />
          </div>
        </div>
        <div class="w-1/2">
          <div class="mb-4">
      ...
      <div class="flex gap-4">
        <div class="w-1/2">
          <div class="mb-4">
            <label for="tmtjabatan" class="block mb-2">Jenis Hukuman</label>
            <FormTextAreaDisabled v-model="data.jenis_hukuman" :rows="1" name="tmtjabatan" id="tmtjabatan" readonly />
          </div>
        </div>
        <div class="w-1/2">
          <div class="mb-4">
            <label for="jenishukuman" class="block mb-2">Nomor SK Jenis Hukuman</label>
            <FormTextAreaDisabled v-model="data.sk_hukuman" :rows="1" name="jenishukuman" id="jenishukuman" readonly />
          </div>
        </div>
      </div>
      <div class="flex gap-4">
        <div class="w-1/2">
          <div class="mb-4">
            <label for="nomorsk" class="block mb-2">TMT Berakhirnya Hukuman Disiplin</label>
            <FormTextAreaDisabled v-model="data.tmt_akhir" :rows="1" name="nomorsk" id="nomorsk" readonly />
          </div>
        </div>
        <div class="w-1/2">
          <div class="mb-4">
            <label for="tmtakhir" class="block mb-2">Nomor SK CLTN</label>
            <FormTextAreaDisabled v-model="data.sk_cltn" :rows="1" name="tmtakhir" id="tmtakhir" readonly />
          </div>
        </div>
      </div>
      <div class="flex gap-4">
        <div class="w-1/2">
          <div class="mb-4">
            <label for="nomorskcltn" class="block mb-2">TMT Berakhir CLTN</label>
            <FormTextAreaDisabled v-model="data.tmt_berakhir" :rows="1" name="nomorskcltn" id="nomorskcltn" readonly />
          </div>
        </div>
        <div class="w-1/2">
          <div class="mb-4">
            <label for="tmtakhircltn" class="block mb-2">Masa Kerja</label>
            <FormTextAreaDisabled v-model="data.masa_kerja" :rows="1" name="tmtakhircltn" id="tmtakhircltn" readonly />
          </div>
        </div>
      </div>
      <div class="flex gap-4">
        <div class="w-1/2">
          <div class="mb-4">
            <label for="masakerja" class="block mb-2">SLKS Yang Diusulkan</label>
            <FormTextAreaDisabled v-model="data.slks_usul" :rows="1" name="masakerja" id="masakerja" readonly />
          </div>
        </div>
      </div>
    </div>
    <div v-if="selectedComponent === 'berkas'">
      <div class="flex justify-between mt-3">
        <p class="font-roboto text-xl text-gray-800 font-semibold mt-5">Berkas Persyaratan</p>
        <button class="bg-blue-500 text-white px-4 py-2 rounded-lg mt-4">Generate Berkas dari SIAP Jabar</button>
      </div>
      <div class="flex flex-col gap-4 mt-5">
        <div v-for="(item, index) in pembekalan" class="bg-white rounded-md p-4 flex w-full h-104 border border-gray-200">
          <div class="flex flex-col items-start justify-start">
              ...
              <p v-if="index === 2" style="color: red; font-size: 12px; margin-left:40px; margin-top: 5px;">Berkas belum tersedia, silahkan update di SIAP Jabar</p>
          </div>
          <div class="flex items-center justify-end flex-grow">
            <button v-if="index !== 2" class="bg-green-500 text-white px-2 py-2 rounded-lg">Download Berkas</button>
            <button v-if="index === 4" class="bg-green-500 text-white px-2 py-2 rounded-lg">Download Berkas</button>
          </div>
        </div>
      </div>
      <hr class="border-gray-300 mt-5" />
      <p class="font-roboto text-xl text-gray-800 font-semibold mt-5"></p>
      <div class="flex flex-col gap-4 mt-5">
        <div v-for="(item, index) in loyalitas" class="bg-white rounded-md p-4 flex w-full h-104 border border-gray-200">
          ...
          <div class="flex items-center justify-end flex-grow">
            <template v-if="isUbahBerkasMode">
              <div style="margin-top: -20px;">
              <FormUploadFile :file="file" @handleFileChange="handleFileChange" />
              </div>
              <button @click="isUbahBerkasMode = false" class="text-red px-4 py-2 rounded-lg">Batal</button>
            </template>
            <template v-else>
              <button class="bg-green-500 text-white px-2 py-2 rounded-lg mr-2">Download Berkas</button>
              <button @click="isUbahBerkasMode = true" class="bg-blue-500 text-white px-4 py-2 rounded-lg">Ubah Berkas</button>
            </template>
          </div>

        </div>
        <FormVueSelect v-model:modelValue="catatan.jenis" class="input-select" label="Jenis Catatan" :options="['Catatan 1', 'Catatan 2', 'Catatan 3']"/>
        <FormTextArea v-model:modelValue="catatan.kirim" label="Kirim Catatan" :rows="6" placeholder="Isilah catatan dengan singkat dan jelas" name="kirim" id="kirim" required />
        <div class="flex justify-end">
          <button class="bg-green-500 text-white px-4 py-2 rounded-lg mt-4">Simpan</button>
        </div>
        <div class="flex justify-end">
